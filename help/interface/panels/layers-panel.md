...
1. O **seletor de modo de mesclagem** permite ajustar como uma camada se mescla com as camadas abaixo dela. O **seletor de modo Combinar** só está disponível quando uma camada de material é selecionada. Os filtros não usam modos de mesclagem.
...
| Ações | Como |
| --- | --- |
| Adicionar uma camada | Arraste um ativo do **painel Ativos** para o visor para adicioná-lo à parte superior da pilha de camadas. Arraste um ativo do **painel Ativos** para a pilha de camadas para adicioná-lo a um local específico na pilha de camadas. Use o botão **Adicionar uma camada** da seção de ferramentas para selecionar um filtro de uma lista. |
...
![Camadas de material no pilha de camadas, a camada superior tem uma opção de mesclagem.](../../assets/Material-Layer.png)

As regras gerais para camadas de material são:

* Uma camada de material sempre usa a resolução do documento.
* Uma camada de material na parte inferior da pilha não tem nada com que se misturar, portanto, o **seletor de modo Combinar** não está disponível.
...
Você pode arrastar uma imagem do navegador de arquivos do sistema para o Sampler. Se já houver camadas na pilha de camadas, a camada de imagem será adicionada ao topo da pilha. Se não existirem camadas na pilha de camadas, irá aparecer uma janela onde poderá escolher como processar a imagem:
...
Você também pode arrastar várias imagens selecionadas para a pilha de camadas de uma só vez para importá-las como uma única camada. Isso pode ser útil para filtros de várias imagens, como **Mesclagem de HDR** e **Multiângulo para material**. Selecione a camada com várias imagens para alterar os dados de canal de cada imagem.
