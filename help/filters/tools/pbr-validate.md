---
helpx_url: "https://helpx.adobe.com/br/substance-3d-sampler/filters/tools/pbr-validate.html"
breadcrumb-title: ''
...
Use o **filtro de Validações do PBR** para garantir que os valores de PBR do material estejam corretos. Ao contrário da maioria dos filtros, o **filtro de Validações do PBR** não deve ser uma parte permanente da pilha de camadas. Em vez disso, use-o para validar seu material e removê-lo para que ele não modifique seu material.
...
* **Modo de Validação**:\
  Selecione se deseja validar valores de albedo (cor base ou difuso), valores metálicos ou valores de albedo e metálico. Outros parâmetros serão atualizados com base nesta seleção
  * **Modo de Validação: Albedo**
    ...
    * **Sobrepor Mapa**: alternar\
      Alternar entre os modos de sobreposição - se ativado, o mapa de cores de base será sobreposto pelos pixels inválidos.
    * **Ocultar validação na cor base**: alternar\
      Ocultar as informações de validação do canal de cores base.
...
No mundo real, existem algumas cores e combinações de propriedades que são impossíveis ou incrivelmente raras. Por exemplo, quase nada no mundo real tem um albedo branco puro ou preto puro ou cor de base.
...
Para usar a **Validação do PBR**, adicione-a ao topo de sua pilha de camadas. Você deve ver uma mudança drástica na aparência do material. Isso ocorre porque o **filtro de Validações do PBR** exibe os resultados da validação no canal de albedo.
