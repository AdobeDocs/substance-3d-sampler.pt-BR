...
Use a **ferramenta Corte demarcado** para ajustar o corte da imagem ou do material. A **ferramenta Cortar** funciona de forma muito semelhante à **ferramenta Transformar**. Com a **ferramenta Transformar**, as alterações feitas na caixa de transformação se comportam de uma para uma maneira com a imagem subjacente; portanto, o aumento da escala da caixa de transformação aumenta o tamanho da imagem subjacente. Com a **ferramenta Cortar**, essa relação é invertida, o aumento da escala da caixa Cortar diminui o tamanho da imagem subjacente. Por esse motivo, ao usar a **Ferramenta Corte demarcado**, pode ser útil definir a **Exibição 2D** para exibir Entradas de Camada, em vez das Saídas de Material padrão.
...
>[!NOTE]
>
> Observe que a **ferramenta Corte demarcado** pode funcionar em imagens ou materiais. Se houver uma imagem ou um canal de digitalização na pilha de camadas sob a **camada Cortar**, o **filtro Cortar** será aplicado ao canal de Digitalização. Se não existir nenhuma imagem ou canal de digitalização, o **filtro de Corte** modificará o material.

Nas imagens abaixo, você pode ver a **Ferramenta Corte demarcado** em ação.

![](../../assets/3d-2d-filters-cropped-0047-crop-in.jpg)

Observe que a exibição 2D está definida para exibir Entradas de Camada para que as alças na **exibição 2D** mostrem qual área da entrada se tornará a saída.
...
* **Transformação de corte**: 0-1\
  Modifique os valores de matriz da transformação. A edição desses valores pode fornecer um controle mais preciso sobre a rotação e o dimensionamento, além de permitir que você incline as alças de corte.
...
Clique na **ferramenta Corte demarcado** para adicionar uma nova camada de filtro de Corte demarcado ao topo da pilha de camadas.

Criar ou selecionar uma camada de filtro de corte abre automaticamente a **exibição 2D**. Com a camada Cortar selecionada, uma barra de ferramentas aparece na parte superior da **exibição 2D**.

## Funcionalidade

>[!NOTE]
>
> O filtro Corte demarcado executa o inverso do movimento, da escala ou da rotação solicitada. Se você achar que o filtro Corte não está correto, pode achar o filtro Transformação mais útil.

### Mover

Para mover a camada:

1. Passe o mouse dentro da caixa de transformação
1. Seu cursor mudará para quatro setas
1. Clique e arraste para mover a caixa de transformação.
...
1. Seu cursor mudará para quatro setas.
1. Clique e arraste para dimensionar a caixa de transformação.

>[!NOTE]
>
> As alças no canto da caixa de transformação permitirão dimensionar em duas dimensões ao mesmo tempo, enquanto as alças na borda da caixa de transformação limitarão o dimensionamento a uma dimensão.

### Girar

Para girar a camada:

1. Passe o mouse fora da caixa de transformação, mas dentro da **exibição 2D**.
1. Uma pequena seta horizontal aparecerá ao lado do cursor.
1. Clique e arraste para girar a caixa de transformação.

>[!NOTE]
>
> É possível alterar o centro da rotação arrastando o pequeno círculo no centro da caixa de transformação. A caixa de transformação sempre gira em torno desse círculo.
...
* Redefinir centro de rotação: redefina o centro de rotação para o centro da caixa Transformar.
