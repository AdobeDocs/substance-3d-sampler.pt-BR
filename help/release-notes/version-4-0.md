...
Com o **Substance 3D Sampler 4.0**, você pode usar imagens do mundo real para criar objetos 3D com mascaramento automático de objetos, mapeamento de texturas e dizimação de geometria. Esta versão apresenta algumas melhorias de UX como novas possibilidades na API Python.
...
Assim que o objeto 3D for reconstruído, otimize o resultado com a dizimação automática, o desembrulho UV e a cozedura.
...
* exportar luzes de ambiente: `export_environment_light`
* exportar malha com ou sem texturas: `export_mesh` ou `export_3d_object`
...
* [captura 3D] Otimizar sua malha e texturas criando uma nova versão
* [captura 3D] Cada versão é automaticamente dizimada para o conjunto de números de faces de destino
* [captura 3D] A etapa de pós-processamento automaticamente desquebra, reprojeta texturas e, em seguida, prepara as informações normais de height e AO da malha de alto polígono
* [captura 3D] Adicione o resultado original ou uma versão ao projeto do Sampler
* [captura 3D] Nova camada de pós-processamento de malha para dizimar, desempacotar, reprojetar texturas e assar automaticamente os detalhes da camada de malha subjacente
* [captura 3D] Nova camada de Transformação de malha para dimensionar, girar ou traduzir a camada de malha subjacente
* [Exportar] Nova janela de exportação
* [Exportar] Configurações dedicadas e interface dependendo do tipo de ativo (material, luz ambiente, malha)
* [Exportar] Exporte a malha como USD, USDA, USDZ, glTF, glb, obj, fbx, stl
...
* [Script] Funções de exportação adicionadas para materiais, luzes ambiente e objetos 3D
* [Script] Identificador adicionado, valor padrão, valores mínimos e máximos, rótulos e valores enum para parâmetros
* [Scripting] Adicionada a função import\_textures para inserir um uso personalizado ao importar imagens
...
* [Parâmetros expostos] A exportação de luzes de ambiente paramétricas não funciona
...
* [IU] Ícone de ativo ausente para luzes ambiente no painel Projeto
...
* [captura 3D] Pequenos artefatos poderão aparecer em seu objeto se a escala na transformação de malha for muito pequena
